	    ...
	    $(function() {
   		...
   		function cariLaporanTHL() {
			...
			if(!inpObj.checkValidity())
				...
				$.ajax({
					url : "<?php echo base_url('cetak/get_laporan_target_bulanan_thl'); ?>",
					type: "POST",
					...
					success: function(data) {
						if(jQuery.isEmptyObject(data.detail_thl)) {
			    			...
			    			$.each(data.laporan_thl, function(i, d) {
			    				target 	= '<ul>';
			    				$.each(d.target, function(ix, dx) {
			    					target 	+= ('<li>' + dx + '</li>');
			    				});
			    				target 	+= '</ul>';

			    				var tanggal = new Date(d.tanggal_target);
		    					...
		    					$('#target-table-body').append(row);
			    				num++;
	                        	aktivitas 	= '';
	                        	status 		= '';
	                        	if(d.kode_kabid)
	                        		last_kabid 	= d.kode_kabid;
							});
							...
							if(data.laporan_thl.length === 0)
								$('#target-table-body').append('<tr><td colspan="4" class="text-center">Tabel Kosong</td></tr>');
						}
					},
