...
class Pelaporan extends MX_Controller
{
	...
	function get_laporan(){
		...
		foreach ($data['laporan_thl'] as $key =>$row) {
			...
			$output['data'][]=array(
				...
				'action' 			=> '<a href="'.base_url("/Pelaporan/detail_aktivitas/".$row['kode_laporan_thl']).'" style="color:white;"><button type=."button" class="btn btn-primary btn-circle"><i class="fas fa-th-list"></i></button></a> '.$btn	
			);
	...
	function tambah_aktivitas() {
		...
		// $this->form_validation->set_rules('tgl_laporan', 'Tanggal Laporan', 'required|is_unique[t_laporan_thl.tgl_laporan]');


		// if ($this->form_validation->run() == FALSE){
		// 	$this->output->set_content_type('json', 'utf-8');
		// 	$this->output->set_status_header(400);
		// 	echo json_encode("Tanggal sudah terdaftar, silahkan pilih tanggal lain!");
		...
		$kode_laporan_thl = create_kode();
			...
			// $kode_thl = "17bquA2p2c4";
		$kode_thl = $this->session->userdata('kode_thl');
			// $result = $this->M_pelaporan->getThl($kode_thl);
			// $kode_spv_kabid=null;$kode_spv_kasie = null;$skpd_id=null;
			// $bidang_skpd_id=null;$profesi_thl_id=null;

			// foreach ($result as $key => $value) {
			// 	$kode_spv_kasie 	= $value['kode_spv_kasie'];
			// 	$kode_spv_kabid		= $value['kode_spv_kabid'];
			// 	$skpd_id			= $value['skpd_id'];
			// 	$bidang_skpd_id		= $value['bidang_skpd_id'];
			// 	$profesi_thl_id		= $value['profesi_thl_id'];
	...
	function detail_aktivitas($kode_laporan_thl){
		...
		$result = $this->M_pelaporan->get_kode_target2($kode_laporan_thl);
		$kode_target_thl = null;
		...
		$data['kegiatan']=$this->M_pelaporan->get_kegiatan_target($kode_target_thl);
	...
	function tabel_aktivitas_lain($kode_laporan_thl){
		...
		foreach ($data['detail'] as $key) {
			...
			$output['data'][]=array(
				...
				'waktu'					=> $key->waktu,
				'status' 				=> '<label class="label label-'.$label[0].'">'.$label[1].'</label>',
				'lampiran' 				=> '<a href="'.site_url().'_upload/'.$key->kode_thl.'/'.$key->kode_laporan_thl.'/'.$key->lampiran.'" target="_blank"><button type="button" class="btn btn-outline-success">show</button></a>',
				'verifikasi'		=> 'Kasie :'.$tgl_kasie.'<br>Kabid :'.$tgl_kabid,
	...
	function tambah_aktivitas_kegiatan(){
		$kode_thl = $this->input->post('kode_thl');
		$kode_laporan_thl = $this->input->post('kode_laporan_thl');
		...
		if ( ! $this->upload->do_upload('lampiran'))
		{
			...
			// echo "<script>swal('Warning message!', 'Upload Lampiran Gagal!!!.');</script>";
			var_dump($error);
		}
		else
		{
			...
			$lampiran = $file_info['file_name'];
			$aktivitas_kegiatan = array(
				'kode_laporan_thl' 		=> xss_filter($kode_laporan_thl),
				'kegiatan_thl_id'		=> xss_filter($this->input->post('kegiatan_thl_id')),
				'stat_laporan_id' 		=> 1,
				'waktu' 				=> xss_filter($waktu),
				'uraian' 				=> xss_filter($this->input->post('uraian')),
				'lampiran' 				=> $lampiran);
			// var_dump($lampiran);

			$data_laporan = $this->M_pelaporan->get_jml_waktu($kode_laporan_thl);
			$jml_waktu = null;
			
			foreach ($data_laporan as $key => $value) {
				$jml_waktu = $value['jml_waktu'];
			}

			$jml_waktu_baru = $jml_waktu + $waktu;

			$jml = array(
				'jml_waktu'=>$jml_waktu_baru);
			$where = array(
				'kode_laporan_thl'=> $kode_laporan_thl);

			$this->M_pelaporan->post_detail_laporan_thl($aktivitas_kegiatan);
	...
	function tambah_aktivitas_lain(){
		...
		else
		{
			...
			$this->M_pelaporan->post_aktivitas_lain($aktivitas_lain);
			$this->M_pelaporan->update_total_jml($where,$jml);
			...
			redirect('pelaporan/detail_aktivitas/'.$kode_laporan_thl);
			// echo "<script>swal('Warning message!', 'Berhasil!!!.');</script>";
		}
	}
